...
#define OWN_IP_ADDRESS   \
	{                    \
		192, 168, 0, 250 \
	} // must be a valid IP address on your LAN
...
int main(void)
{
	...
	printk("own mac address is %u.%u.%u.%u.%u.%u \n",
	...
	while (1)
	{
		...
		if (pARPFrame->Ethernet.nProtocolType != BE(ETH_PROT_ARP) || pARPFrame->ARP.nHWAddressSpace != BE(HW_ADDR_ETHER) || pARPFrame->ARP.nProtocolAddressSpace != BE(PROT_ADDR_IP) || pARPFrame->ARP.nHWAddressLength != MAC_ADDRESS_SIZE || pARPFrame->ARP.nProtocolAddressLength != IP_ADDRESS_SIZE || pARPFrame->ARP.nOPCode != BE(ARP_REQUEST))
		{
			continue;
		}
